<template>
            ...
            <v-row no-gutters class="align-start">
              ...
              <v-col class="info-col">
                <div class="nickname-row">
                  ...
                  <v-btn class="unfollow-btn" @click="removeFollowing(user.id)">언팔로우</v-btn>
                </div>
  ...
  const fetchFollowings = async () => {
    try {
      ...
      followings.value = followingData.map(user => ({
        id: user.id,
  ...
  const visibleFollowings = computed(() => {
    return filteredFollowings.value.slice(0, visibleCount.value)
  })
